...
CSV_FILE = BASE_DIR / "jackery_log.csv"


def load_data():
    """CSVファイルからデータを読み込んで辞書形式で返す"""
    ...
    try:
        ...
        for row in rows:
            ...
            for col in columns:
                raw = row.get(col, '')
                # OutputAC / OutputDC は True/False 文字列 → 0/1
...
:root {
  ...
  --border-a: rgba(0,212,255,0.3);
  --t1: #e4eeff;
  --t2: #8899bb;
  --t3: #4a5878;
  --ac: #00d4ff;
  --ac-d: rgba(0,212,255,0.15);
  --gr: #00e676;
  --gr-d: rgba(0,230,118,0.15);
  --or: #ff6d3b;
  --ye: #ffd740;
  --re: #ff5252;
  --pu: #b388ff;
...
body {
  ...
  min-height: 100vh;
  background-image:
    radial-gradient(ellipse at 15% 50%, rgba(0,212,255,0.05) 0%, transparent 55%),
    radial-gradient(ellipse at 85% 15%, rgba(179,136,255,0.05) 0%, transparent 55%);
}
header {
...
.rbtn {
  background: var(--ac-d); border: 1px solid var(--border-a);
  color: var(--ac); padding: 7px 15px; border-radius: 8px;
...
.card {
  ...
  border-radius: 16px; padding: 18px 15px; transition: all .2s;
...
.card.ac .cv { color: var(--ac); }
.card.gr .cv { color: var(--gr); }
.card.or .cv { color: var(--or); }
.card.ye .cv { color: var(--ye); }
.card.re .cv { color: var(--re); }
.card.pu .cv { color: var(--pu); }
.badge {
...
.tbtn .dot2 { width: 8px; height: 8px; border-radius: 50%; animation: none; }
...
.loverlay {
  ...
  z-index: 200; align-items: center; justify-content: center; font-size: 15px; color: var(--t2);
}
...
@media (max-width: 480px) {
  ...
  h1 { font-size: 16px; }
  #chart { height: 300px; }
  .chdr, .toggles { padding: 13px 14px; }
}
</style>
</head>
<body>
...
function renderChart(){
  ...
  const layout = {
    ...
    xaxis: {
      ...
      rangeslider: { bgcolor: 'rgba(255,255,255,0.03)', bordercolor: 'transparent', thickness: 0.06 },
    },
    yaxis: {
      gridcolor: 'rgba(255,255,255,0.06)',
      linecolor: 'rgba(255,255,255,0.1)',
    ...
    hoverlabel: {
      ...
      font: { family: 'Inter, sans-serif', size: 12, color: '#e4eeff' }
    },
  };
...
def get_data():
    data = load_data()
    if data is None:
        return jsonify({"error": "データがありません。jackery_api.py が実行されているか確認してください。"})
